...
class ArchiveView {
    ...
    fun generateArchivePage(archivedJobs: List<ArchivedJob>): String {
        ...
        return buildString {
            ...
            appendLine("                } else {")
            appendLine("                    // Пытаемся получить JSON ответ, если возможно")
            appendLine("                    let errorMessage = `HTTP \${response.status}: \${response.statusText}`;")
            appendLine("                    try {")
            appendLine("                        const result = await response.json();")
            appendLine("                        if (result.error) {")
            appendLine("                            errorMessage = result.error;")
            appendLine("                        }")
            appendLine("                    } catch (jsonError) {")
            appendLine("                        // Если не удается распарсить JSON, используем статус ответа")
            appendLine("                        console.log('Failed to parse JSON response:', jsonError);")
            appendLine("                    }")
            appendLine("                    alert('Ошибка при удалении: ' + errorMessage);")
            appendLine("                }")
